      ...
      <el-card shadow="never" class="login-card">
        ...
        <el-form ref="formData" :model="formData" :rules="formRules">
          <el-form-item prop="mobile">
            <el-input v-model.trim.number="formData.mobile" placeholder="请输入您的手机号" />
          </el-form-item>
...
export default {
    ...
    return {
      ...
      formRules: {
        mobile: [
          ...
          { pattern: /^1[3-9]\d{9}$/, message: '请正确填写手机号', trigger: 'blur' }
        ],
        password: [
  ...
  methods: {
    login() {
      this.$refs.formData.validate(async(value) => {
        if (value) {
          // 登录请求，获取token
